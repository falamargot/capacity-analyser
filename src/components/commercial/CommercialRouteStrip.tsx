...
function CommercialRouteStrip({
  ...
  return (
    <div className={`commercial-route-strip border-t border-[rgba(148,163,184,0.08)] ${compact ? 'bg-[rgba(6,10,22,0.70)] px-3 py-1.5' : 'bg-[rgba(6,10,22,0.94)] px-4 py-1.5'} backdrop-blur-xl`}>
      {!compact && (
      ...
      <div className={`flex items-stretch ${compact ? 'gap-1' : 'gap-1.5'}`}>
        {displayedSegments.map((segment, index) => {
          ...
          const label = compact ? compactJourneyLabel[segment.type] : journeyLabel[segment.type];
          return (
          <div key={segment.id} className={`flex min-w-0 items-center ${compact ? 'gap-1' : 'gap-1.5'} ${isOutcome ? 'flex-[1.18]' : 'flex-1'}`}>
            <button
              type="button"
              onClick={() => onSelectedSegmentChange(segmentId)}
              data-access={isAccess ? 'true' : 'false'}
              data-outcome={isOutcome ? 'true' : 'false'}
              data-primary-issue={isPrimaryIssue ? 'true' : 'false'}
              data-selected={isSelected ? 'true' : 'false'}
              className={[
                'commercial-route-strip__card flex w-full min-w-0 flex-col justify-center rounded-lg border text-left transition-colors',
                compact ? 'min-h-[2.1rem] px-2 py-1' : 'px-2.5 py-1.5',
                isSelected
                  ? isAccess
                    ? `${compact ? '' : 'min-h-[3.25rem]'} border-cyan-200/90 bg-cyan-400/20 text-white shadow-[0_0_0_1px_rgba(103,232,249,0.34),0_0_22px_rgba(34,211,238,0.28)]`
                    : `${compact ? '' : 'min-h-[3.15rem]'} border-sky-300/80 bg-sky-500/20 text-white shadow-[0_0_0_1px_rgba(125,211,252,0.28),0_0_18px_rgba(56,189,248,0.22)]`
                  : isPrimaryIssue
                    ? `${compact ? '' : 'min-h-[2.95rem]'} border-amber-400/60 bg-amber-500/12 text-white shadow-[0_0_0_1px_rgba(251,191,36,0.22),0_0_14px_rgba(251,191,36,0.16)]`
                    : isOutcome
                      ? `${compact ? '' : 'min-h-[2.95rem]'} border-sky-500/30 bg-[rgba(15,23,42,0.70)] text-white hover:bg-[rgba(15,23,42,0.90)]`
                      : isAccess
                        ? `${compact ? '' : 'min-h-[2.95rem]'} border-cyan-400/22 bg-cyan-400/8 text-slate-200 hover:bg-cyan-400/12`
                      : `${compact ? '' : 'min-h-[2.95rem]'} border-[rgba(51,65,85,0.55)] bg-[rgba(15,23,42,0.55)] text-slate-300 hover:bg-[rgba(15,23,42,0.80)]`,
              ].join(' ')}
            ...
            {!compact && index < displayedSegments.length - 1 && (
              <ChevronRight className={`${compact ? 'h-3.5 w-3.5' : 'h-4 w-4'} commercial-route-strip__chevron shrink-0 text-slate-300 dark:text-slate-700`} />
            )}
          </div>
          );
        })}
