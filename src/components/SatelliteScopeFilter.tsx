...
const SatelliteScopeFilter: React.FC<SatelliteScopeFilterProps> = ({
  ...
  return (
    <div className={`${compact ? 'grid w-full grid-cols-3 rounded-[22px] p-1 shadow-sm' : 'flex rounded-lg p-0.5'} bg-gray-100 dark:bg-slate-800 border border-gray-200 dark:border-slate-700`}>
      {scopes.map((scope) => (
        <button
          key={scope}
          ...
          className={`
            ...
            ${compact ? 'flex min-w-0 items-center justify-center rounded-[16px] px-3 py-2.5 text-[13px]' : 'px-4 py-2 text-sm'}
            ${currentScope === scope
