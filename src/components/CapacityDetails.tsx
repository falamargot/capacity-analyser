...
const CapacityDetails = memo<CapacityDetailsProps>(({ satellites, selectedPoint, selectedSatellite, autoSelectedLEOSatellite, autoSelectedGEOSatellite, satelliteScope, onSelectedGEOBeamChange, analysisSource, aircraftCallsign, selectedSNP: propSelectedSNP }) => {
  ...
  const [lastWeatherSource, setLastWeatherSource] = useState<'auto' | 'manual'>('auto');
  ...
  if (!selectedPoint && !selectedSatellite) {
    return (
      <div className="h-full bg-white rounded-lg shadow-lg p-6 flex items-center justify-center text-gray-500">
        ...
        <div className="flex-1 min-h-0 overflow-y-auto">
          <div className="mb-4">
            <div className="bg-gray-50 rounded-lg p-3">
              <h3 className="text-sm font-semibold mb-2 text-gray-800">User Terminal</h3>
              <div className="space-y-3">
                <div className="flex items-center space-x-3">
                  <label className="text-sm font-medium text-gray-600 w-16">Type:</label>
                  <select
                    value={terminalType}
                ...
                <div className="flex items-center space-x-3">
                  <label className="text-sm font-medium text-gray-600 w-16">Weather:</label>
                  <select
                    value={weatherType}
                    onChange={(e) => {
                    ...
                    style={{
                      backgroundImage: terminalType === 'aviation' ? 'none' : `url("data:image/svg+xml;charset=US-ASCII,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 4 5'%3E%3Cpath fill='%236B7280' d='M2 0L0 2h4zm0 5L0 3h4z'/%3E%3C/svg>")`,
                      backgroundRepeat: 'no-repeat',
                      backgroundPosition: 'right .5rem center',
                      backgroundSize: '.8em .8em',
                  ...
                  <label className={`flex items-center space-x-1 text-xs whitespace-nowrap ${terminalType === 'aviation' ? 'text-gray-400' : 'text-gray-500'}`}>
                    <input
                      type="checkbox"
                      checked={autoWeatherEnabled}
                      onChange={(e) => {
                        setAutoWeatherEnabled(e.target.checked);
                        setLastWeatherSource(e.target.checked ? 'auto' : 'manual');
                      }}
                      disabled={terminalType === 'aviation'}
                    />
                    <span>Auto</span>
                      ...
                      return (
                        <div className="space-y-3">
                          <div className="flex justify-between items-center">
                            ...
                            <span className="text-sm font-semibold text-gray-900">{performance.rtt} ms</span>
                          </div>
                ...
                <div className="bg-gray-50 rounded-lg p-4 mt-1">
                  ...
                  {resolvedGEOConnectivity ? (
                    <div className="text-sm text-gray-700 text-left space-y-3">
                      <div>User → {resolvedGEOConnectivity.satellite.name} → User</div>
                      <div className="text-xs text-gray-600 space-y-2">
                        <div>
                          <div>User → {resolvedGEOConnectivity.satellite.name}</div>
                          <div className="ml-4">↳ Beam: {resolvedGEOConnectivity.beam?.name?.split('-').pop()?.trim() || 'Default Beam'} | Elevation: {resolvedGEOConnectivity.elevation.toFixed(1)}° | Distance: {Math.round(resolvedGEOConnectivity.distance)} km ({computeOneWayLatencyMs(resolvedGEOConnectivity.distance)} ms)</div>
                        </div>
                      </div>
                    </div>
                  ) : (
                    <div className="text-sm text-gray-700 text-left">
                      <div>No GEO beam coverage for this location.</div>
                    </div>
                  )}
                </div>
                {/* GEO Estimated Performance */}
                <div className="bg-gray-50 rounded-lg p-4">
                  <h4 className="text-sm font-semibold mb-3" style={{ color: '#2563eb' }}>Estimated Performance (Theoretical)</h4>
                  {resolvedGEOConnectivity ? (
                    (() => {
                      const perf = calculateGEOPerformance(resolvedGEOConnectivity.elevation);
                      return (
                        <div className="space-y-3">
                          <div className="flex justify-between items-center">
                            <span className="text-sm font-medium text-gray-600">Round-trip latency (RTT):</span>
                            <span className="text-sm font-semibold text-gray-900">{resolvedGEOConnectivity.rtt} ms</span>
                          </div>
                          <div className="flex justify-between items-center">
                            <span className="text-sm font-medium text-gray-600">Downlink throughput:</span>
                            <span className="text-sm font-semibold text-gray-900">
                              {perf.performanceFactor > 0
                                ? (perf.downlinkGbps >= 1
                                    ? `${perf.downlinkGbps.toFixed(1)} Gbps`
                                    : `${Math.round(perf.downlinkGbps * 1000)} Mbps`)
                                : 'Not usable'}
                            </span>
                          </div>
                          <div className="flex justify-between items-center">
                            ...
                            <span className="text-sm font-semibold text-gray-900">
                              {perf.performanceFactor > 0
                                ? (perf.uplinkGbps >= 1
                                    ? `${perf.uplinkGbps.toFixed(1)} Gbps`
                                    : `${Math.round(perf.uplinkGbps * 1000)} Mbps`)
                                : 'Not usable'}
                            </span>
                          </div>
                          <div className="flex justify-between items-center">
          ...
          {/* 4) Capacity Context - Compact Footer */}
          {selectedPoint && (
            <div className="text-xs text-gray-500 text-center py-2 space-y-1">
              <div>
