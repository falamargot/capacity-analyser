...
const GEOConnectivitySection = memo<GEOConnectivitySectionProps>(({
  ...
  // NOTE: this is a beam-route-level failover signal, distinct from
  // commercialRouteModel.ts's `controlAssignmentRole === 'backup'` check on the
  // resolved gateway itself — both currently agree in the common path because
  // analyzeGeoConnectivity() funnels through the same beam-aware resolver, but
  // they are two independently-named booleans for the same concept. See the
  // dev-only `logStarGatewayCanaryDev` canary, which already monitors this
  // class of divergence. Cross-Surface Consistency Audit 2026-07-21, M4.
  ...
  // Node-based route diagram — kept structurally symmetric with LEO's F2/M1
  // fix (Cross-Surface Consistency Audit 2026-07-21) even though GEO's own
  // text summary was never confirmed buggy (a single GEO satellite genuinely
  // serves both Mesh/P2P sites), so the two Inspector files stay visually and
  // structurally aligned going forward.
  const geoRouteDiagram = buildGeoRouteDiagram({
    isMeshOrP2P,
    isStarReturn,
    isForwardMeshDirection: activeMeshTab === 'forward',
    onSatelliteClick,
    satellite: resolvedGEOConnectivity?.satellite ?? null,
    satelliteDisplayName: drawerSatelliteName,
    star: !isMeshOrP2P ? {
      userLabel,
      gatewayDisplayName,
      userToSatKm: geoGeometry?.userToSatellite.slantRangeKm ?? null,
      userToSatMs: geoGeometry?.userToSatellite.latencyMs ?? null,
      satToGatewayKm: geoGeometry?.satelliteToGateway.slantRangeKm ?? null,
      satToGatewayMs: geoGeometry?.satelliteToGateway.latencyMs ?? null,
      userElevationLabel: geoGeometry ? `${geoGeometry.userToSatellite.elevationDeg.toFixed(1)}°` : undefined,
      userCoverageLabel: isStarReturn
        ? (formatCoverageName(selectedUplinkCoverage) ?? resolvedGEOConnectivity?.candidate.coverageName)
        : (formatCoverageName(selectedDownlinkCoverage ?? selectedCoverage) ?? resolvedGEOConnectivity?.candidate.coverageName),
    } : undefined,
    mesh: isMeshOrP2P && meshGeometry ? {
      pointALabel: meshGeometry.pointALabel,
      pointBLabel: meshGeometry.pointBLabel,
      aToSatKm: meshGeometry.aToSatKm,
      aToSatMs: meshGeometry.aToSatMs,
      satToBKm: meshGeometry.satToBKm,
      satToBMs: meshGeometry.satToBMs,
      bToSatKm: meshGeometry.bToSatKm,
      bToSatMs: meshGeometry.bToSatMs,
      satToAKm: meshGeometry.satToAKm,
      satToAMs: meshGeometry.satToAMs,
      aElevationLabel: `${meshGeometry.elevA.toFixed(1)}°`,
      bElevationLabel: `${meshGeometry.elevB.toFixed(1)}°`,
      aCoverageLabel: meshGeometry.beamNameAtA,
      bCoverageLabel: meshGeometry.beamNameAtB,
    } : undefined,
  });
  const pathSummaryEvidence = (
    <div className="space-y-2.5">
      <RouteDiagram
        technology="GEO"
        ariaLabel="GEO resolved route"
        nodes={geoRouteDiagram.nodes}
        connectors={geoRouteDiagram.connectors}
      />
