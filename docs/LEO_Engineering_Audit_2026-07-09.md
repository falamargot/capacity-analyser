...
> **✅ IMPLEMENTED 2026-07-10** (on top of the Lot-1 tree). Gates: **778/778 tests** (net +18 incl. new bottleneck/cache/SNP/latency regressions), `tsc` clean, lint at the pre-existing 4-warning baseline (stash-verified), build OK, **Playwright smoke passed** (Paris single-site: panel renders, evidence trace live — ONEWEB-0011 via SNP Mornac, 13 Mbps / 48 ms, bottleneck "DL+UL beam sharing", 0 app console errors, evidence build avg 0.111 ms/call with the cache). Details:
> - **L-M3/L-Mo8**: `leoRouteAnalysisViewModel.ts` deleted (544 lines, zero importers). CapacityDetails' ~900-line fallback pipeline deleted (`computedLeoPerformance`, `computedLeoSiteToSiteResult`, `debugInfoB` builder, EMA refs, both approximate/beam-aware callbacks, local bottleneck trio) — `leoPerformance`/`leoSiteToSiteResult` now read exclusively from the App evidence. New `leoBottleneck.ts` is the single detector with **table-derived** C/N thresholds (`deriveModcodBottleneckThresholds`: upper-half boundary + top entry reproduce the former 14.5/18.5 literals and track table swaps).
> - **L-M4**: `calculateCombGeometry` + `calculateCombBeamCenters` gained single-entry-per-satrec WeakMap caches keyed (timeMs, sim-state signature incl. beam health); `leoHasCurrentRF` now reads `leoConnectivityStatus.hasRFConnectivity` (memo deleted); `leoSiteBHasCurrentRF` memo deleted (dev trace reads the evidence). Also fixed the evidence input-signature bug (`beamHealthFactors` — a non-existent field — replaced with serialized `beamHealthByIndex` entries).
> - **L-M5**: `selectBestBeamIndexByNormalizedDistance` ranks by the canonical 51 × 800 km ellipse; `calculateLink`, `getBestBeamLink`, `getPhysicsAwareBeamRadius` (the 317-km circular per-beam model) deleted.
> - **L-Mo5**: new `selectSnpForSatellite` in connectivityRules (max feeder elevation, GSO-gated, outage-aware) is used by auto-resolution (`assessGatewayLinks` delegates), the App inspection memo, and SatelliteDetails (label now "Serving feeder SNP (highest elevation)"); `getNearestSNPInBackhaul` and coverageService's private 3-D distance copy deleted.
> - **L-Mo3**: `S2S_ONE_WAY_PROCESSING_MS` (28 ms) derived from `DEFAULT_LEO_OVERHEAD_MS` (2 modems + 2 gateways + routing + queueing); single-site SNP→PoP fiber comes from `estimateSnpToPopFiberOneWayMs` (same PoP catalog + route factor as the S2S backbone, 5 ms floor); `expectedRttMinMs` recalibrated 65 → 45; dead `handoverRiskMarginMs` removed.
> - **L-Mi2/3/4/6**: one GSO pitch/blanking implementation (`gsoProtection.ts`, worker-safe; consumed by oneWebComb, oneWebCombCore and the SatelliteDetails pitch chart); one `getRadiusAtPowerLevel` (`leoBeamPattern.ts`, worker-safe); beam constants single-sourced from config/oneweb (67.5/16 literals gone); `countActiveBeams` helper replaces the four inline derivations; inverted "cross-track/along-track" comments corrected everywhere (beams stack ALONG-track, elongate CROSS-track); dead exports deleted (`selectBestServingCandidate`+margins, `SNR_ROLLOFF_ZONES`/`throughputRatioFromPowerDb`, `capDeliveredToTerminal`, `calculateLinkQuality`, `getAllBeamCharacteristics`, `BEAM_BW_SCALE`s, `capacityLimitation`).
> - **L-Mo9**: verified App already keeps the per-site weather knobs and simulation snapshots in sync (Site A ↔ SimulationContext, Site B derived from `weatherTypeB`); documented the invariant on the evidence input and added a DEV drift canary.
