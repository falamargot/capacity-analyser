...
test.describe('REVISIT P2c-A selected result', () => {
  test('keeps the reference as benchmark while the selected point owns the result', async ({ page }, testInfo) => {
    ...
    await expect(analysis.getByLabel('Active result context')).toContainText('Point result · Reference target');
    ...
    await expect(analysis.getByLabel('Active result context')).toContainText('Point result · Comparison target');
    ...
    await expect(analysis).toContainText('Comparison target location required');
    ...
    // The comparison target's own sweep is lazy, so the curve arrives after the
    ...
    if (testInfo.project.name === 'mobile-chromium') {
      ...
      await expect(page.locator('[data-revisit-result-strip]')).toContainText('Comparison target · max gap');
    }
  });
  ...
  test('selects a comparison from the aligned comparison table', async ({ page }, testInfo) => {
    ...
    await page.locator('[data-revisit-context-panel="analysis-target"]')
      .getByRole('button', { name: /Reference target/ }).click();
    ...
    await expect(page.getByLabel('Active result context')).toContainText('Point result · Comparison target');
